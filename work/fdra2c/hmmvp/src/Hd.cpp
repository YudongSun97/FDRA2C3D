...
namespace hmmvp {
  ...
  Block::Block(uint r0_, uint m_, uint c0_, uint n_)
  ...
  // matrix.
  Block::Block(const Vecui& ip, const Vecui& iq, const Vecui& r, const Vecui& c)
  {
    r0 = MinBlockIdx(ip, r);
    m = r.size();
    c0 = MinBlockIdx(iq, c);
  ...
  void MajorAxis(const Matd& A, Matd& u)
  {
    ...
    Matd U;
#ifdef HMMVP_MEXSVD
    // For Matlab user's convenience, don't have to link to LAPACK and BLAS
    MexSvd(B, &U, 0, 0);
#else
    Matd s, Vt;
    Svd(B, U, s, Vt);
  ...
  public:
    ClusterTree
    (ClusterDataFactory* cdf, HdCc::SplitMethod sm, const Matd* A,
     const Matd* wgt = 0);
    virtual ~ClusterTree();
  ...
  (ClusterDataFactory* cdf, HdCc::SplitMethod sm, const Matd* A,
   const Matd* wgt)
    : cdf(cdf), split_method(sm), A(*A), root(0), min_points(3)
  {}
  ...
  public:
    ...
    double Distance(const HdCcCDPointwise& cdp) const;
  ...
  // returned.
  double HdCcCDPointwise::Distance(const HdCcCDPointwise& cdp) const
  {
    const Matd& A = *this->A;
  ...
  vector<Block>::size_type Hd::NbrBlocks() const { return blocks.size(); }
  ...
  bool HdCc::Admissible(const HdCcCD& n1cd, const HdCcCD& n2cd) const
  {
    ...
    case distCentroid: {
      ...
      double r1 = n1.Radius(), r2 = n2.Radius(), dist = 0.0;
      for (int i = 1; i <= nd; i++) {
        double a = c1(i) - c2(i);
        dist += a*a;
      }
    ...
    default: {
      ...
      return min(n1.Diameter(), n2.Diameter()) <= eta*n1.Distance(n2);
    } break;
    }
  }
  ...
  {
    Block b(permpi, permpi, n1->Idxs(), n2->Idxs());
    ...
    if (Admissible(static_cast<const HdCcCD&>(n1->CD()),
        ...
        // proved that the two algorithms give the same results. The matter is
        // one of academic, but not practical, interest restricted just to the
        // decomposition.) as in the original version of HdCcSym::Compute below.
        !(hc1 || hc2)) {
#else
      // This condition makes each block cluster tree node always have 0 or 4
      // children. This is what we want if we intend to obtain an LU
      // factorization using the resulting block cluster tree.
      !hc1 || !hc2) {
#endif
      blocks.push_back(b);
    ...
    for (int i = 0, m1 = ns1.size(); i < m1; i++) {
      ...
      for (int j = 0, m2 = ns2.size(); j < m2; j++) {
        ...
        if (Admissible(static_cast<const HdCcCD&>(n1->CD()),
          ...
          blocks.push_back(Block(permpi, permpi, n1->Idxs(), n2->Idxs()));
          blocks.push_back(Block(permpi, permpi, n2->Idxs(), n1->Idxs()));
        } else {
          // n2 is too near n1
    ...
    for (int i = 0, m1 = ns1.size(); i < m1; i++) {
      ...
      for (int j = 0, m2 = ns2.size(); j < m2; j++) {
        ...
        if (Admissible(static_cast<const HdCcCD&>(n1->CD()),
                       static_cast<const HdCcCD&>(n2->CD()))) {
          // n2 is far enough away from n1
          blocks.push_back(Block(permpi, permqi, n1->Idxs(), n2->Idxs()));
        } else {
          // n2 is too near n1
          bool hc2 = !n2->Chldrn().empty();
          if (hc1 || hc2) {
  ...
  namespace {
    bool IsSame(const Matd& D, const Matd& R)
    {
  ...
  {
    return NewHd(D, Matrix<double>());
  }

  void WriteHd(const Hd* hd, const std::string& hd_filename)
    throw (FileException)
  {
    FILE* fid = fopen(hd_filename.c_str(), "w");
  ...
  {
    FILE* fid = fopen(hd_filename.c_str(), "r");
    if (!fid) throw FileException();
    ...
    try {
      ...
      for (uint i = 0, k = 0; i < n; i++, k += 4)
        hd->AppendBlock(Block(data[k], data[k+1], data[k+2], data[k+3]));
