...
mxArray* Mvp
(hmmvp::Hmat& h, const double* x, uint m, uint ncol,
 const vector<hmmvp::Blint>* rs = 0, const vector<hmmvp::Blint>* cs = 0)
{
  if (h.GetN() != m || ncol == 0)
...
int Init(const char *filename, uint nthreads, uint ncol, uint& nnz)
{
  ...
  if (id == -1) {
    ghms.push_back((hmmvp::Hmat*)0);
    id = ghms.size() - 1;
  }

  try {
    ghms[id] = hmmvp::NewHmat(string(filename), ncol, nthreads);
    nnz = ghms[id]->GetNnz();
...
inline hmmvp::Hmat& GetHmat(const mxArray* ma)
{
