#ifndef INCLUDE_UTIL_RECTMESHUNSTRUCT
#define INCLUDE_UTIL_RECTMESHUNSTRUCT
...
namespace util {
  namespace rmesh {
    ...
    struct RectOpts {
      ...
      // Resolution at which to check f.
      double check_res;

      RectOpts() : min_len(0.0), max_len(0.0), check_res(0.0) {}
      RectOpts(FILE* fid);
      RectOpts(double imin_len, double imax_len, double icheck_res)
        : min_len(imin_len), max_len(imax_len), check_res(icheck_res) {}
      void Serialize(FILE* fid) const;
      void Deserialize(FILE* fid);
    };
    ...
    class UniformMeshLinInterpRF : public ResolutionFn {
    public:
      UniformMeshLinInterpRF(const Rect& r, const Matrix<double>& f)
        : _r(r), _f(f) {}
