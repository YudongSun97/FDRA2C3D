...
# define MPI_SUCCESS 0
  ...
  namespace mpi {
    ...
    public:
      ...
      // a file sing fread and then sending.
    ...
    public:
      ...
      // View the buffer, interpreted as type T, from any position pos, where
      // pos refers to the position in bytes. No bounds checking.
      template<typename T> const T* View(size_t pos) const;
      // View a scalar.
      template<typename T> T ViewScalar(size_t pos) const;

      // Mix of View and Read: view the data, and advance the buffer pointer by
      // n*sizeof(T).
      template<typename T> const T* ViewAndAdvance(size_t n);
      // View and advance by sizeof(T).
