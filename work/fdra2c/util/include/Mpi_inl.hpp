...
namespace util {
  namespace mpi {
      ...
      if (_all_equal_size)
        ...
        if (mpi::AmRoot()) {
          ...
          if ((ret = mpi::Waitall(nproc - 1, &_reqs[0], &_stats[0])))
            return ret;
          // Is this necessary? I don't think so, but I want to make sure I
          // don't get leaks.
    ...
    {
      if (init_sz == 0) init_sz = 1;
      _buf.reserve(init_sz);
    }
    ...
    template<typename T>
    inline void ByteBufferWriter::WriteScalar(T data)
    {
      Write(&data, 1);
    }
    ...
    {
      size_t nbytes = n * sizeof(T);
      while (_buf.capacity() < _buf.size() + nbytes)
        _buf.reserve(2 * _buf.capacity());
      size_t i = _buf.size();
    ...
    template<typename T>
    inline void ByteBufferWriter::Resize(size_t n)
    {
      _buf.resize(n);
    }
    ...
    template<typename T>
    inline const T* ByteBufferReader::View(size_t pos) const
    {
      return (const T*)(_buf + pos);
    }
    ...
    template<typename T>
    inline const T* ByteBufferReader::ViewAndAdvance(size_t n)
    {
      const T* data = (const T*)(_buf + _i);
      _i += n*sizeof(T);
      return data;
    }
